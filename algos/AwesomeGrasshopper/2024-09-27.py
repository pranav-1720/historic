...
def p(
    ...
    if not first_day:
        ...
        if first_time_opponent_out_of_stock == -1:
            if not competitor_has_capacity_current_period_in_current_season:
                first_time_opponent_out_of_stock = selling_period_in_current_season
                price += 20
            ...
            information_dump['history'].append(
                {
                    ...
                    'first time opponent out of stock': first_time_opponent_out_of_stock
                },
                ignore_index=True
            )
        )
